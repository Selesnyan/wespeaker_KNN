...
def run_epoch(dataloader,
    ...
    alpha = 0.2  # weight of loss functions
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast(enabled=enable_amp):
            ...
            if use_per_frame_embeddings:
                ...
                #targets_sos = (targets != 0).float().to(device)
                
                #print(outputs.shape, targets.flatten().shape, logits_sos.shape, targets_sos.view(-1, 1).shape)
                #loss_sos = criterion_sos(logits_sos, targets_sos.view(-1, 1))
                loss_sid = criterion(outputs, targets)
                
                loss = loss_sid # alpha * loss_sos + (1 - alpha) * loss_sid
