...
from wespeaker.dataset.dataset_sc import Dataset ###
...
from wespeaker.utils.executor_sc import run_epoch
...
def spk2id_rttm(rttm_file):
    labels = {}
    with open(rttm_file, 'r') as f:
        for line in f.readlines():
            spkid = line.split(' ')[7]
            labels[spkid] = 1
    return {label: i for i, label in enumerate(sorted(labels.keys()))}
...
def train(config='conf/config.yaml', **kwargs):
    ...
    """
    print(configs['data_type'], configs['train_data'])

    train_label = configs['train_label']
    if os.path.isfile(train_label + '.pkl'):
        print('loading rttm from pickle')
        with open(train_label + '.pkl', 'rb') as f:
            rttm_dict = pickle.load(f)
    else:
        print('loading rttm')
        rttm_dict = load_rttm(train_label)
    spk2id_dict = spk2id_rttm(train_label)

    train_utt_spk_list = list(rttm_dict.keys())  # only len is used
    #####

    print(len(train_utt_spk_list), len(spk2id_dict))
    if rank == 0:
    ...
    if configs['data_type'] != 'feat' and configs['dataset_args'][
        ...
        #configs['projection_args']['num_class'] += 1
    ...
    configs['projection_args']['num_class'] = 1
    configs['projection_args']['project_type'] = "linear" # default
    model.add_module("projection_speech", get_projection(configs['projection_args']))
    if rank == 0:
        # print model
    ...
    for epoch in range(start_epoch, configs['num_epochs'] + 1):
        ...
        run_epoch(train_dataloader,
                  ...
                  rank=rank,
                  wandb=None,
                  use_per_frame_embeddings=configs.get('use_per_frame_embeddings', False),
                  use_lennorm_embeddings=configs.get('use_lennorm_embeddings', False),
